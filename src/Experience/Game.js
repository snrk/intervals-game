...
export default class Game {
  constructor() {
    ...
    this.experience.on('demo', (intervalData) =>
      this.runDemo([intervalData])
    )
    this.experience.on('play', (intervalData) =>
      this.enterPlayState([intervalData])
    )
  }
  ...
  enterSettingState() {
    ...
    this.experience.camera.targetOffset.copy(
      this.experience.camera.camOffsets.setting
    )
  ...
  runDemo(intervalData) {
    ...
    const baseNoteX =
      this.experience.world.ui.getKeyPositionX(baseNoteIndex)
    const intervalNoteX =
      this.experience.world.ui.getKeyPositionX(intervalNoteIndex)

    this.demoTimeline = gsap.timeline({
      onComplete: () => {
        // Send base note and interval key's X position
        this.experience.trigger('play', [
          { baseNoteX, intervalNoteX },
        ])
        this.demoTimeline = null
      },
    })
  ...
  enterPlayState(notesToPlay) {
    ...
    this.playTimeline
      ...
      .call(
        () => {
          this.experience.world.stage.animBallBase(
            this.data.baseNoteX,
            ballSpeed
          )
        },
      ...
      .call(
        () => {
          this.experience.world.stage.animBallInterval(
            this.data.intervalNoteX,
            ballSpeed
          )
        },
        [],
  ...
  updateRingPosition(midiNote) {
    ...
    const positionX =
      noteDifference * this.experience.KEY_WIDTH +
      this.experience.KEY_WIDTH * 0.5
    // Update RingPosition
