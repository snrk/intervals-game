...
export default class Loading extends EventEmitter {
    ...
    showMicMessage() {
        ...
        gsap.to(this.micMessage.material, {
            ...
            onComplete: () => {
                ...
                this.canvas.addEventListener('click', () => this.startXP() , { once: true })
            }
        })
    }
    ...
    hidePermissionUI() {
        gsap.to(this.micMessage.material, { opacity: 0, duration: 0.5, ease: 'power2.out', onComplete: () => { this.micMessage.visible = false } })
        
        if (!this.experience.sizes.isPortrait) 
            gsap.to(this.overlayMaterial.uniforms.uAlpha, { value: 0, delay: .3, duration: 0.8, ease: 'power2.out' })
    }
    ...
    update() {
        if (this.gKey) {
            this.visualProgress += (this.targetProgress - this.visualProgress) * 0.5 
            this.gKey.material.uniforms.uProgress.value = this.visualProgress
        }
    }
}