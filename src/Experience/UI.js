...
export default class UI {
    ...
    setCurveArrow() {
        ...
        this.curveArrowPlus.material = this.curveArrowPlus.material.clone()
    ...
    setCurveArrowPosition () {
        this.curveArrowPlus.position.set(-.16, .16, 0)
        this.curveArrowMinus.position.set(.16, .16, 0)
    }
    ...
    startTutorialAnimation() {
        ...
        // Toturial Timeline
        tl.to(this.tutoTitle.scale, { delay:0.5, duration: 0.5, x: 200, y: 200, z: 200, ease: 'power1.inOut' }, "start")
        .fromTo(this.tutoTexte.scale,
            { x: 0, y: 0, z: 0 },
            { duration: 0.5, x: 200, y: 200, z: 200, ease: 'power1.inOut' },
            "start+=1.5")
        .fromTo(this.lineArrow.scale,
            { x: 0, y: 0, z: 0 },
            { duration: 0.5, x: 200, y: 200, z: 200, ease: 'power1.inOut' },
            "start+=1.9"
        )
        .fromTo(this.curveArrowMinus.scale,
            { x: 0, y: 0, z: 0 },
            { duration: 0.5, x: 0.66, y: 0.66, z: 0.66, ease: 'power1.inOut' },
            "start+=2" 
        )
        .fromTo(this.curveArrowPlus.scale,
            { x: 0, y: 0, z: 0 },
            { duration: 0.5, x: -0.66, y: 0.66, z: 0.66, ease: 'power1.inOut' },
            "start+=2" 
        )
        .to([this.lineArrow.scale, this.curveArrowPlus.scale, this.curveArrowMinus.scale, this.tutoTexte.scale], {
            duration: 0.3,
            x: 0,
            y: 0,
            z: 0,
        ...
        .to(this.tutoTitle.material.map.offset, {
            duration: .4,
            y: 0.5,
            ease: 'power1.out'
        }, "start+=5.2")
   ...
   setGameTextes() {
    ...
    showText(text) {
        ...
        // Set texture
        this.gameTextes.material.map.repeat.x = .97
        ...
        this.textTimeline
            // In
            .to(this.gameTextes.scale, { duration: 0.3, x: 200, y: 200, z: 200, ease: 'back.out' })
            // Out
            .to(this.gameTextes.scale, { duration: 0.3, x: 0, y: 0, z: 0, ease: 'back.in' }, "+=.4")
    ...
    setBackButtonPosition () {
        this.backButton.position.set(
            this.experience.sizes.width * .5 - 150,
            this.experience.sizes.height * .5 - 50,
            -50)
    }
    playBackButton() {
    ...
    setOrientationMessage() {
        this.landscapeMessage = this.model.getObjectByName("landscapeMessage")
     ...
     */
    setIntervalsButton() {
        this.intervalsButton = this.model.getObjectByName("buttonIntervals")
    ...
    setDiskIntervals() {
        ...
        INTERVALS.forEach(interval => {
            ...
            if(mesh) {
                ...
                mesh.hoverTimeline = gsap.timeline({ paused: true })
                .fromTo(mesh.scale, { x: diskScale, y: diskScale, z: diskScale }, { duration: .2, x: diskScaleHover, y: diskScaleHover, z: diskScaleHover, ease: "power2.inOut" })
     ...
     */
    setFullscreenButton () {
        this.fullscreenButton = this.model.getObjectByName("button-fullscreen")
    ...
    setFullscreenButtonPosition () {
        this.fullscreenButton.position.set(
            this.experience.sizes.width * .5 - 50,
            this.experience.sizes.height * .5 - 50,
            -50)
    }
    ...
    setTimelinesButton() {
        ...
        // Back Button
        this.backButtonHover = gsap.timeline({ paused: true })
            .fromTo(this.backButton.scale, { x: 300, y: 300, z: 300, }, { duration: .5, x: 350, y: 350, z: 350, ease: "power2.inOut", }, "<")

        // Fullscreen
        this.fullscreenHover = gsap.timeline({ paused: true })
            .fromTo(this.fullscreenButton.scale, { x: 300, y: 300, z: 300}, { duration: .2, x: 350, y: 350, z: 350, ease: "power2.inOut", }, "<")

        // Play Intervals
        this.intervalsHover = gsap.timeline({ paused: true })
            .fromTo(this.intervalsButonGroup.scale, { x: 1, y: 1, z: 1}, { duration: .2, x: 1.1, y: 1.1, z: 1.1, ease: "power2.inOut", }, "<")
         ...
         */
        // Fullscreen
        this.fullscreenClick = gsap.timeline({ paused: true })
            .fromTo(this.fullscreenButton.rotation, { x: 0,}, { duration: .5, x: Math.PI, ease: "power2.inOut", }, "<")
        ...
        // Play Intervals
        this.intervalsIn = gsap.timeline({ paused: true })
            .fromTo(this.intervalsButton.scale, { x: 0, y: 0, z: 0, }, { delay: 0.4, duration: .5, x: 300, y: 300, z: 300, ease: "power2.inOut", }, "<")
