...
export default class Camera {
  ...
  setInstance() {
    ...
    if (this.debug.active) {
      this.debugFolder
        .add(this.basePosition, 'x')
        .name('camPosX')
        .min(-50)
        .max(50)
        .step(0.1)
      this.debugFolder
        .add(this.basePosition, 'y')
        .name('camPosY')
        .min(-50)
        .max(50)
        .step(0.1)
      this.debugFolder
        .add(this.basePosition, 'z')
        .name('camPosZ')
        .min(-200)
        .max(200)
        .step(0.1)
  ...
  setCamerasPosition() {
    ...
    if (this.instance.aspect <= 1) {
      this.instance.aspect = THREE.MathUtils.clamp(
        this.instance.aspect,
        0.4,
        1
      )
      depthPosition = THREE.MathUtils.mapLinear(
        this.instance.aspect,
        0.4,
        1,
        195,
      ...
      heightPosition = THREE.MathUtils.mapLinear(
        this.instance.aspect,
        0.4,
        1,
        65,
      ...
      rotationPan = THREE.MathUtils.mapLinear(
        this.instance.aspect,
        0.4,
        1,
        -0.2,
  ...
  endDrag() {
    // Snap to key
    const closestNoteIndex = Math.round(
      Math.abs(this.dragOffset.x) / this.KEY_WIDTH
    )
  ...
  setCameraOffsets() {
    ...
    this.experience.on('menu', () => {
      this.targetOffset.copy(this.camOffsets.menu)
    })
    // 'setting' position call in Game constructor
    this.experience.on('demo', () => {
      this.targetOffset.copy(this.camOffsets.demo)
    })
    this.experience.on('play', () => {
      this.targetOffset.copy(this.camOffsets.play)
    })
  }
