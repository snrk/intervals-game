...
export default class Stage {
    ...
    setBalls() {
        ...
        this.ballIntervalTemplate = this.model.getObjectByName('ballInterval')           
    }
    animBallBase(xPosition, duration) {
        this.ballBaseTemplate.position.x = xPosition
        this.ballBaseIn = gsap.timeline({ paused: true })
            .fromTo(this.ballBaseTemplate.position, { z: -20}, { duration: duration, z: 5}, "<")
        this.ballBaseIn.play()
    }
    animBallInterval(xPosition, duration) {
        this.ballIntervalTemplate.position.x = xPosition
        this.ballIntervalIn = gsap.timeline({ paused: true })
            .fromTo(this.ballIntervalTemplate.position, { z: -20}, { duration: duration, z: 5 }, "<")
        this.ballIntervalIn.play() 
    }

    midiToPositionX(midiNote) {
        const noteDifference = midiNote - this.experience.game.baseOctaveNote - 24
    ...
    setModel() {
        ...
        this.model.traverse((child) => {
            if(child instanceof THREE.Mesh) {
                ...
                // Debug
        if (this.debug.active) {
            this.debugFolder.add(this.sceneGroup.position, 'x').name('scenePosX').min(-1000).max(1000).step(1)
        }
    }
    ...
    setKeyboard() {
        ...
        this.raycastKeyboardIn = gsap.timeline({ paused: true })
            .fromTo(this.raycastKeyboard.position, { y: -10}, { duration: .2, y: 0.14534, ease: "power2.inOut", }, "<")
    }
