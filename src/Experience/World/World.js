...
export default class World {
  ...
  mouseRecording(_event) {
    this.mouse.x =
      (_event.clientX / this.experience.sizes.width) * 2 - 1
    this.mouse.y =
      -(_event.clientY / this.experience.sizes.height) * 2 + 1
  }
  ...
  onPointerDown(_event) {
    ...
    this.ray.setFromCamera(
      this.mouse,
      this.experience.camera.instanceOrtho
    )
    const intersectsUI = this.ray.intersectObjects(
      this.objectsForTestingOrtho
    )
    ...
    const intersects3D = this.ray.intersectObjects(
      this.objectsForTestingPersp
    )
  ...
  onPointerUp(_event) {
    ...
    // CLICK
      ...
      this.currentIntersect.object === this.clickedObject
    ) {
      this.handleClick()
    }
  ...
  handleHover() {
    ...
    const intersectedObject = intersectsUI.length
      ? intersectsUI[0].object
      : null
    ...
    if (
      ...
      // Timelines Hover IN
      if (this.currentIntersect.hoverTimeline)
        this.currentIntersect.hoverTimeline.play()
      else if (this.currentIntersect === this.ui.fullscreenButton)
        this.ui.fullscreenHover.play()
      else if (this.currentIntersect === this.ui.intervalsButton)
        this.ui.intervalsHover.play()
      else if (this.currentIntersect === this.ui.backButton)
        this.ui.backButtonHover.play()
    }
    // Hover OUT
      ...
      // Timelines Hover OUT
      if (this.currentIntersect.hoverTimeline)
        this.currentIntersect.hoverTimeline.reverse()
      else if (this.currentIntersect === this.ui.fullscreenButton)
        this.ui.fullscreenHover.reverse()
      else if (this.currentIntersect === this.ui.intervalsButton)
        this.ui.intervalsHover.reverse()
      else if (this.currentIntersect === this.ui.backButton)
        this.ui.backButtonHover.reverse()
  ...
  handleClick() {
    if (this.currentIntersect) {
      ...
      // Interval Disk Button
        ...
        this.ui.intervalGroup.children.includes(objectClicked)
      ) {
